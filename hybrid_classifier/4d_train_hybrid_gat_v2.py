"""
GAT-based Hybrid GCN with Node-Specific Features + Global Context (Option 2)
- Node features: Per-node geometric data (x, y, visibility, angles, distances)
- Global features: Hybrid similarity scores
- Architecture: GAT processes nodes with attention, then combines with global features
"""
...
class HybridGAT(nn.Module):
    ...
    - GAT processes spatial node features with attention
    ...
    def __init__(self, node_in_channels, hybrid_in_channels, hidden_channels, num_classes, heads=4, dropout=0.5):
        ...
        # Fusion layer (combines GAT output + hybrid features)
    ...
    def forward(self, x, edge_index, batch, hybrid_features):
        ...
        # Fusion: concatenate GAT output + hybrid features
...
def load_hybrid_graph_data(viewpoint_filter=None):
    ...
    for i in range(len(train_data['node_features'])):
        node_feat = train_data['node_features'][i]
        hybrid_feat = train_data['hybrid_features'][i]
...
def train_hybrid_gat(viewpoint_filter=None, epochs=150, lr=0.001, hidden_dim=128, heads=4, dropout=0.5):
    """Train Hybrid GAT with node + global features"""
    ...
    print(f"\n{'='*60}")
    print(f"Training Hybrid GAT (Option 2){viewpoint_str}")
    print(f"Architecture: Node Features (GAT) + Global Features (MLP)")
    print(f"Heads: {heads}, Hidden: {hidden_dim}, Dropout: {dropout}")
    print(f"{'='*60}\n")
    ...
    for epoch in range(epochs):
        ...
        if test_acc > best_acc:
            ...
            torch.save({
                ...
                'heads': heads,
                'dropout': dropout
            }, OUTPUT_DIR / f"hybrid_gat_v2{suffix}.pth")
        else:
            patience_counter += 1
...
if __name__ == "__main__":
    ...
    for vp in viewpoints_to_train:
        print(f"\n{'='*60}")
        print(f"TRAINING HYBRID GAT V2: {vp.upper()}")
        print(f"{'='*60}")
