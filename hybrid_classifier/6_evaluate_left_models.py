"""
Phase 6: Real-Only Evaluation of Left Viewpoint Models
Evaluate all 4 trained models on real (non-synthetic) test data.
...
# Same skeleton edges as training
SKELETON_EDGES = [
...
class GraphDataset(Dataset):
    def __init__(self, features_path, viewpoint=None, filter_nan=True):
        ...
        # Node mask for v6 models (1.0 for body nodes, 0.0 for stick nodes if no stick detected)
...
def load_model(model_path, model_version, device):
    ...
    # Infer dimensions from checkpoint if possible
    state_dict = ckpt['model_state_dict']
    
    # Get node feature dim from first conv weight
    node_feat_dim = None
    hybrid_feat_dim = None
    for key, val in state_dict.items():
        if 'convs.0.lin.weight' in key or 'convs.0.lin_rel.weight' in key:
            # GCNConv weight shape: [out_dim, in_dim]
            node_feat_dim = val.shape[1] - 8  # subtract node_embed_dim=8
            break
        elif 'convs.0.weight' in key:
            node_feat_dim = val.shape[1] - 8
            break
    
    # Get hybrid dim from hybrid_mlp first layer
    for key, val in state_dict.items():
...
def evaluate_model(model, dataset, device, batch_size=64):
    ...
    # Per-class confusion (most confused classes)
    confusion_pairs = []
    for i in range(NUM_CLASSES):
...
def main():
    ...
    models = [
        {
            'name': 'v5_standard',
            'model_path': 'hybrid_classifier/models/model_left_with_synthetic_3x_v5_standard.pth',
            'test_path': 'hybrid_classifier/hybrid_features_v5/test_features_left_standard_v5.pt',
            'version': 'v5',
            'template': 'standard'
        },
        {
            'name': 'v5_mirrored',
            'model_path': 'hybrid_classifier/models/model_left_with_synthetic_3x_v5_mirrored.pth',
            'test_path': 'hybrid_classifier/hybrid_features_v5/test_features_left_mirrored_v5.pt',
            'version': 'v5',
            'template': 'mirrored'
        },
        {
            'name': 'v6_standard',
            'model_path': 'hybrid_classifier/models/model_left_with_synthetic_2x_v6_standard.pth',
            'test_path': 'hybrid_classifier/hybrid_features_v6/test_features_left_standard_v6.pt',
            'version': 'v6',
            'template': 'standard'
        },
        {
            'name': 'v6_mirrored',
            'model_path': 'hybrid_classifier/models/model_left_with_synthetic_2x_v6_mirrored.pth',
            'test_path': 'hybrid_classifier/hybrid_features_v6/test_features_left_mirrored_v6.pt',
            'version': 'v6',
            'template': 'mirrored'
        }
    ]
    ...
    for model_info in models:
        ...
        # Load dataset
        dataset = GraphDataset(model_info['test_path'], viewpoint='left', filter_nan=True)
    ...
    output_path = Path('hybrid_classifier/models/left_viewpoint_real_only_evaluation.json')
