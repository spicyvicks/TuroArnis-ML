"""
Advanced Hybrid GCN with Graph Attention Networks (GAT)
Research-backed: Learns which edges (e.g., hand-stick) are most important
Based on: Shi et al. (2019) - Skeleton-Based Action Recognition with Spatial Reasoning
"""
...
class AttentionGCN(nn.Module):
    """
    Graph Attention Network for Skeleton-Based Recognition
    ...
    def __init__(self, in_channels, hidden_channels, num_classes, heads=4, dropout=0.5):
        ...
        # GAT Layer 2 (single head for classification)
...
def train_gat_gcn(viewpoint_filter=None, epochs=150, lr=0.001, hidden_dim=64, heads=4, dropout=0.5):
    """Train GAT-based GCN"""
    ...
    print(f"\n{'='*60}")
    print(f"Training GAT-GCN{viewpoint_str}")
    ...
    for epoch in range(epochs):
        ...
        if test_acc > best_acc:
            ...
            torch.save({
                ...
                'heads': heads,
                'dropout': dropout
            }, OUTPUT_DIR / f"gat_gcn{suffix}.pth")
        else:
            patience_counter += 1
...
if __name__ == "__main__":
    ...
    for vp in viewpoints_to_train:
        print(f"\n{'='*60}")
        print(f"TRAINING GAT MODEL: {vp.upper()}")
        print(f"{'='*60}")
