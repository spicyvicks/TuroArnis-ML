...
OUTPUT_DIR = Path("hybrid_classifier/hybrid_features_v6")
...
def extract_raw_features(image_path, stick_detector, viewpoint=None, class_idx=None):
    ...
    if stick_results.keypoints is not None and len(stick_results.keypoints.data) > 0:
        ...
        # ALL classes now use TRUE zero-stick fallback instead of rejecting the image
    ...
    # 14. Left wrist horizontal position — person-normalized
    # Thrusts: far left (negative), Blocks: near center (~0), Neutral: near hip
