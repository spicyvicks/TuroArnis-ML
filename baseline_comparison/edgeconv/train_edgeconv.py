"""
EdgeConv (Dynamic Graph CNN) Baseline for Arnis Pose Classification
Tests learned graph topology vs fixed skeleton structure
"""
import torch
...
OUTPUT_DIR = Path("baseline_comparison/edgeconv/results")

class EdgeConvNet(nn.Module):
    def __init__(self, k=20, num_classes=12):
        super().__init__()
        self.k = k
        
        # EdgeConv: MLP processes concatenated [x_i, x_j - x_i]
        self.conv1 = EdgeConv(nn.Sequential(
            nn.Linear(12, 64),   # 6 + 6 = 12 (features + relative)
            nn.ReLU(),
            nn.BatchNorm1d(64),
            nn.Linear(64, 64)
        ))
    ...
    def forward(self, x, batch_idx):
        # x: (batch*35, 6), batch_idx: batch assignment per node
        
        # Layer 1: k-NN in input space
        edge_index = knn_graph(x, k=self.k, batch=batch_idx)
        x = self.conv1(x, edge_index)  # (N, 64)
        x = F.relu(x)
        
        # Layer 2: recompute graph in learned feature space
        edge_index = knn_graph(x, k=self.k, batch=batch_idx)
        ...
        # Layer 3: final graph
        edge_index = knn_graph(x, k=self.k, batch=batch_idx)
        x = self.conv3(x, edge_index)  # (N, 128)
        
        # Global max pooling per graph
        x = global_max_pool(x, batch_idx)  # (batch_size, 128)
        
        return self.classifier(x)

def to_pyg_batch(X):
    """Convert (batch, 35, 6) tensor to PyG batch"""
    data_list = []
    for i in range(X.size(0)):
        data_list.append(Data(x=X[i], pos=X[i][:, :3]))  # pos for k-NN if needed
...
def train_edgeconv(viewpoint_filter=None, epochs=150, lr=0.001, batch_size=32, k=20):
    """Train EdgeConv classifier"""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print("="*60)
    print(f"EdgeConv (Dynamic Graph CNN) Baseline Training (Device: {device})")
    ...
    # Initialize model
    model = EdgeConvNet(k=k, num_classes=len(CLASS_NAMES)).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_loader:
            ...
            pyg_batch = to_pyg_batch(batch_x)
            pyg_batch = pyg_batch.to(device)
            
            optimizer.zero_grad()
            outputs = model(pyg_batch.x, pyg_batch.batch)
        ...
        if test_acc > best_test_acc:
            ...
            # Save best model
            suffix = f"_{viewpoint_filter}" if viewpoint_filter else ""
            torch.save(model.state_dict(), OUTPUT_DIR / f"edgeconv_best{suffix}.pth")
        else:
            patience_counter += 1
    ...
    suffix = f"_{viewpoint_filter}" if viewpoint_filter else ""
    model.load_state_dict(torch.load(OUTPUT_DIR / f"edgeconv_best{suffix}.pth"))
    ...
    # Save results
    results = {
        'model': 'EdgeConv (Dynamic Graph CNN)',
...
def plot_training_history(history, suffix=""):
    ...
    ax1.set_ylabel('Loss')
    ax1.set_title('EdgeConv - Training History (Loss)')
    ax1.legend()
    ...
    ax2.set_title('EdgeConv - Training History (Accuracy)')
    ax2.legend()
...
def plot_confusion_matrix(y_true, y_pred, suffix=""):
    ...
    plt.title('EdgeConv - Normalized Confusion Matrix')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train EdgeConv baseline")
    ...
    parser.add_argument('--k', type=int, default=20,
                        help='Number of k-NN neighbors (default: 20)')
    
    args = parser.parse_args()
    train_edgeconv(args.viewpoint, args.epochs, args.lr, args.batch_size, args.k)
