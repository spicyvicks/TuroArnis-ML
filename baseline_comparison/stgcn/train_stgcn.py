"""
ST-GCN (Spatio-Temporal Graph CNN) Baseline for Arnis Pose Classification
Tests deep residual graph architecture with fixed skeleton topology
Adapted for static pose (T=1) to compare with Hybrid GCN
"""
import torch
...
OUTPUT_DIR = Path("baseline_comparison/stgcn/results")
...
class GraphConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # Reshape for matrix mult: (N*C*T, V) @ (V, V) → (N*C*T, V)
...
class STGCNBlock(nn.Module):
    """ST-GCN block with residual connection"""
...
class STGCN(nn.Module):
    def __init__(self, num_classes=12, num_joints=35, 
        ...
        # ST-GCN layers: 9 layers with residual connections
        self.st_gcn_networks = nn.ModuleList([
            STGCNBlock(6, 64, A, stride=1, residual=False),
            STGCNBlock(64, 64, A, stride=1),
            STGCNBlock(64, 64, A, stride=1),
            STGCNBlock(64, 64, A, stride=1),
            STGCNBlock(64, 128, A, stride=1),
            STGCNBlock(128, 128, A, stride=1),
            STGCNBlock(128, 128, A, stride=1),
            STGCNBlock(128, 256, A, stride=1),
    ...
    def _create_adjacency(self, num_joints):
        ...
        # Define skeleton edges (33 body joints + 2 stick nodes)
        # MediaPipe 33 body keypoints connections
        edges = [
            # Face
...
def train_stgcn(viewpoint_filter=None, epochs=150, lr=0.001, batch_size=32):
    """Train ST-GCN classifier"""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print("="*60)
    print(f"ST-GCN (Spatio-Temporal GCN) Baseline Training (Device: {device})")
    print("Static mode: T=1 (single frame)")
    print("="*60)
    ...
    for epoch in range(epochs):
        ...
        if test_acc > best_test_acc:
            ...
            # Save best model
            suffix = f"_{viewpoint_filter}" if viewpoint_filter else ""
            torch.save(model.state_dict(), OUTPUT_DIR / f"stgcn_best{suffix}.pth")
        else:
            patience_counter += 1
    ...
    suffix = f"_{viewpoint_filter}" if viewpoint_filter else ""
    model.load_state_dict(torch.load(OUTPUT_DIR / f"stgcn_best{suffix}.pth"))
...
def plot_training_history(history, suffix=""):
    ...
    ax1.set_ylabel('Loss')
    ax1.set_title('ST-GCN - Training History (Loss)')
    ax1.legend()
    ...
    ax2.set_title('ST-GCN - Training History (Accuracy)')
    ax2.legend()
...
def plot_confusion_matrix(y_true, y_pred, suffix=""):
    ...
    plt.title('ST-GCN - Normalized Confusion Matrix')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train ST-GCN baseline")
    ...
    train_stgcn(args.viewpoint, args.epochs, args.lr, args.batch_size)
