"""
Feature Extraction for HybridGCN V5 Deployment
Matches training pipeline: 6-dim nodes + 46 hybrid features (33 base + 13 signed)
Stick fallback: origin-based dist_to_hip/angle (v5 behavior that achieves 77.7%)
"""
...
def extract_raw_features(image, stick_detector):
    ...
    Returns:
        dict with pose_keypoints, stick_keypoints, global_features
    ...
    if stick_results.keypoints is not None and len(stick_results.keypoints.data) > 0:
        ...
        # V5 behavior: origin-based fallback (creates constant offset model learns)
...
def extract_node_features(pose_keypoints, stick_keypoints):
    ...
    V5 behavior: For missing stick nodes (origin fallback), dist_to_hip and angle_from_hip
    are computed from origin (0,0) — this creates a constant offset the model learned
    to compensate for, achieving 77.7% real-only accuracy.
    
    Args:
        pose_keypoints: [33, 4] array from MediaPipe
        stick_keypoints: [2, 4] array (origin-based fallback if not detected)
    ...
    for i, kpt in enumerate(all_keypoints):
        ...
        # V5 behavior: compute dist_to_hip and angle_from_hip even for origin fallback
